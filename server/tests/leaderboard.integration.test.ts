...
describe('GET /api/leaderboard', () => {
    ...
    it('returns exactly ten entries and includes the current user separately', async () => {
        const pointsByPosition = [
            100, 90, 80, 70, 60, 50, 40, 30, 20, 10, 10, 5,
        ]
        ...
        expect(
            response.body.entries.map((entry: { userId: number }) => entry.userId),
        ).toEqual(userIds.slice(0, 10))
        ...
        expect(
            response.body.entries.some(
                (entry: { userId: number }) => entry.userId === currentUserId,
            ),
        ).toBe(false)
    })
