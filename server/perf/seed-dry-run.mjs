...
export async function runRollbackSeedDryRun(client) {
  ...
  if (!client || typeof client.query !== 'function') {
    throw new PerfSafetyError('Rollback dry-run requires a guarded staging client')
  }
  ...
  try {
    ...
    if (finalClock.seedDate !== seedDate) {
      throw new PerfSafetyError('Database date changed during the rollback rehearsal')
    }
    ...
    const restoredDataset = await assertApprovedSeedSourceDataset(client)
    ...
    if (!sourceIdentityRestored) {
      throw new PerfSafetyError('Approved source identity differs after rollback')
    }

    const restoredSnapshot = await captureCoreDatasetSnapshot(client)

    const restorationVerified = coreSnapshotsMatch(originalSnapshot, restoredSnapshot)
    ...
    return {
      ...
      restoration: {
        ...
        originalCounts: {
          ...
          characters: originalSnapshot.characters,
        },
        restoredCounts: {
