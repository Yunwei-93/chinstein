import { connectToStaging, PerfSafetyError, safeError } from './staging-guard.mjs'
import {
  HISTORY_DAYS,
  CHARACTER_COUNT,
  POOLS,
...
try {
  ...
  const matchesExactArgs = (expectedArgs) =>
    ...
    args.every((argument, index) => argument === expectedArgs[index])

  const planRequested = args.length === 0 || matchesExactArgs(['--plan'])

  const dryRunArgs = ['--dry-run', '--confirm-aws-staging', '--confirm-ecs-zero']
  ...
  const perfReseedCommitRequested = matchesExactArgs(perfReseedCommitArgs)

  const commitSeedRequested = initialCommitRequested || perfReseedCommitRequested

  const expectedSourceClassification = initialCommitRequested
    ? 'approved-initial-staging'
    : perfReseedCommitRequested
      ? 'approved-perf-source'
      : null
  if (!planRequested && !dryRunRequested && !commitSeedRequested) {
    throw new PerfSafetyError('Use --plan or one exact confirmed seed mode')
  }
  ...
  `,
      ...
      if (Number(currentData.characters) !== CHARACTER_COUNT) {
        throw new PerfSafetyError('Character count differs from the frozen 365-character fixture')
      }
      ...
      if (!permissions.can_truncate_study_sessions || !permissions.can_create_mapping_tables) {
        throw new PerfSafetyError('Required staging dataset-maintenance permissions are missing')
      }
        ...
        JSON.stringify(
          {
            ...
            proposedReset: {
              ...
              analyzeAfterSeeding: ['public.study_sessions', 'public.characters', 'public.users'],
            },
          },
          null,
          2,
  ...
  console.error(JSON.stringify(safeError(error, stage), null, 2))
  ...
  if (connection) {
    try {
      ...
      console.error(JSON.stringify(safeError(error, 'close-connection'), null, 2))
