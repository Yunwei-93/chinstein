...
export async function connectToStaging() {
    ...
    if (hostFingerprint !== EXPECTED_HOST_FINGERPRINT) {
        throw new PerfSafetyError(
            "Staging host fingerprint changed; refusing to connect"
        );
    }

    if (database !== EXPECTED_DATABASE) {
        throw new PerfSafetyError(
            "Database name differs from the verified staging database"
        );
    }

    if (!url.hostname.includes("-pooler")) {
        throw new PerfSafetyError(
            "Connection is not the verified pooled staging endpoint"
        );
    }
