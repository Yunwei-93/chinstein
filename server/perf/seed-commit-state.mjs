...
export async function readApprovedStateWithFreshConnection({ expectedClassification = null } = {}) {
  let connection
...
export async function verifySourceRestoredWithFreshConnection({
  ...
  if (!expectedDataset) {
    throw new PerfSafetyError('Source restoration verification requires an expected dataset')
  }
  ...
  const identityRestored = approvedSeedSourcesMatch(expectedDataset, observed.dataset)

  const snapshotRestored =
    expectedSnapshot === null ? null : coreSnapshotsMatch(expectedSnapshot, observed.snapshot)

  if (!identityRestored) {
    throw new PerfSafetyError('Approved source identity was not restored after rollback')
  }

  if (snapshotRestored === false) {
    throw new PerfSafetyError('Core source data was not restored after rollback')
  }
...
export async function classifyAmbiguousCommit({
  ...
  if (!sourceSnapshot || !targetSnapshot) {
    throw new PerfSafetyError('Ambiguous commit classification requires both snapshots')
  }

  try {
    const observed = await readApprovedStateWithFreshConnection()

    const sourceIdentityVisible = approvedSeedSourcesMatch(sourceDataset, observed.dataset)
    const sourceSnapshotVisible = coreSnapshotsMatch(sourceSnapshot, observed.snapshot)
    const targetIdentityVisible = approvedSeedSourcesMatch(targetDataset, observed.dataset)
    const targetSnapshotVisible = coreSnapshotsMatch(targetSnapshot, observed.snapshot)

    const sourceVisible = sourceIdentityVisible && sourceSnapshotVisible
    const targetVisible = targetIdentityVisible && targetSnapshotVisible
    ...
    return {
      ...
      observerErrorCode: typeof error?.code === 'string' ? error.code : null,
      fingerprintsPrinted: false,
    }
  }
}
