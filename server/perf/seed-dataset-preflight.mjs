...
const PERF_MAPPING_RELATIONS = ['perf_characters', 'perf_users']
...
async function assertPerfMappingColumnContract(client) {
  ...
  if (!columnsMatch) {
    throw new PerfSafetyError('PERF mapping table columns do not match the approved contract')
  }
...
async function assertPerfFixtureUsers(client) {
  ...
  if (
    ...
    throw new PerfSafetyError('PERF source user ownership checks')
  }
...
async function assertPerfFixtureCharacters(client) {
  ...
  if (
    ...
    Number(state.distinct_sequences) !== CHARACTER_COUNT ||
    Number(state.distinct_characters) !== CHARACTER_COUNT ||
    Number(state.minimum_sequence) !== 1 ||
    Number(state.maximum_sequence) !== CHARACTER_COUNT ||
    Number(state.dangling_mappings) !== 0 ||
    ...
    throw new PerfSafetyError('PERF fixture character ownership checks failed')
  }
  ...
  return {
    ...
    mappingDifferences: Number(state.mapping_differences),
  }
}
...
async function inferPerfSourceSeedDate(client) {
  const result = await client.query(
    ...
    `,
    [POOLS.B.firstSeq, POOLS.B.lastSeq, HISTORY_DAYS, HISTORY_DAYS + 1],
  )

  const state = result.rows[0]

  const expectedPoolBSessions = POOLS.B.users * (HISTORY_DAYS + 1)
  ...
  if (
    ...
    Number(state.sessions) !== expectedPoolBSessions ||
    !state.minimum_anchor_date ||
    state.minimum_anchor_date !== state.maximum_anchor_date ||
    state.source_date_not_in_future !== true ||
    Number(state.users_with_invalid_session_count) !== 0 ||
    Number(state.users_with_invalid_date_range) !== 0
  ) {
    throw new PerfSafetyError('PERF source seed-date anchor checks failed')
  }

  return {
    sourceSeedDate: state.maximum_anchor_date,
...
export async function inspectPerfMappingObjects(client) {
  ...
  if (objects.length !== 0 && objects.length !== PERF_MAPPING_RELATIONS.length) {
    throw new PerfSafetyError('Only one PERF mapping object exists; refusing cleanup')
  }
  ...
  for (const object of objects) {
    if (object.relkind !== 'r') {
      failures.push(`${object.relation_name}:not-an-ordinary-table`)
    }

    if (!object.can_select) {
      failures.push(`${object.relation_name}:not-readable`)
    }

    if (!object.can_drop) {
      failures.push(`${object.relation_name}:not-removable`)
    }
  }

  if (failures.length > 0) {
    throw new PerfSafetyError(`PERF mapping checks failed: ${failures.join(', ')}`)
  }

  return {
    mappingState: objects.length === 0 ? 'absent' : 'present',
...
export async function assertApprovedInitialDataset(client) {
  ...
  if (mappings.mappingState !== 'absent') {
    throw new PerfSafetyError('Initial seed requires both PERF mapping tables to be absent')
  }
  ...
  const result = await client.query(
    ...
    `,
    ['generating', '%@perf.invalid'],
  )

  const state = result.rows[0]

  if (
    Number(state.users) !== APPROVED_INITIAL_DATASET.users ||
    Number(state.study_sessions) !== APPROVED_INITIAL_DATASET.studySessions ||
    Number(state.characters) !== APPROVED_INITIAL_DATASET.characters
  ) {
    throw new PerfSafetyError('Staging no longer matches the approved initial dataset')
  }

  if (Number(state.generating_stories) !== 0) {
    throw new PerfSafetyError('Story generation is currently in progress')
  }

  if (Number(state.synthetic_users) !== 0) {
    throw new PerfSafetyError('Unexpected synthetic users already exist')
  }
  ...
  return {
    ...
    characters: Number(state.characters),
    generatingStories: Number(state.generating_stories),
    syntheticUsers: Number(state.synthetic_users),
...
export async function assertApprovedPerfSourceDataset(client) {
  ...
  if (mappings.mappingState !== 'present') {
    throw new PerfSafetyError('Repeat seed requires both PERF mapping tables')
  }
  ...
  if (
    ...
    sessions.poolBFixtureDateSessions !== TODAY_SESSIONS
  ) {
    throw new PerfSafetyError('PERF source pool distribution is invalid')
  }
  ...
  if (
    ...
    sessions.sessionsBeforeHistory !== 0 ||
    sessions.sessionsAfterFixtureDate !== 0 ||
    sessions.sessionsOutsideDateRange !== 0
  ) {
    throw new PerfSafetyError('PERF source contains sessions outside its fixture dates')
  }

  if (sessions.unmappedUserSessions !== 0 || sessions.unmappedCharacterSessions !== 0) {
    throw new PerfSafetyError('PERF source contains unmapped session records')
  }

  if (sessions.invalidHistoricalSessions !== 0 || sessions.invalidReseedFixtureDateSessions !== 0) {
    throw new PerfSafetyError('PERF source contains non-canonical session records')
  }
...
const APPROVED_SEED_SOURCE_CLASSIFICATIONS = new Set([
...
export function approvedSeedSourcesMatch(expected, actual) {
  return (
    JSON.stringify(approvedSeedSourceIdentity(expected)) ===
    JSON.stringify(approvedSeedSourceIdentity(actual))
  )
}
...
export async function assertApprovedSeedSourceDataset(
  ...
  if (
    expectedClassification !== null &&
    !APPROVED_SEED_SOURCE_CLASSIFICATIONS.has(expectedClassification)
  ) {
  ...
  const selectedClassification =
    mappings.mappingState === 'absent' ? 'approved-initial-staging' : 'approved-perf-source'
  ...
  if (selectedClassification === 'approved-initial-staging') {
