...
if [ "$SUITE" = "timed" ] && [ "$RUN_KIND" = "confirmation" ] && [ -z "$LEVELS_OVERRIDE" ]; then
...
if [ "$SUITE" = "write" ] || [ "$SUITE" = "final" ]; then
  ...
  for repetition in 1 2 3; do
    for vus in 5 10 20 40 80; do
      if PERF_P3_VUS="$vus" \
        PERF_P3_REPETITION="$repetition" \
        k6 run /app/perf/p3/k6/write-run-runtime.js
      then
        :
      else
        run_exit_code="$?"
        ...
        if [ "$run_exit_code" -eq 99 ]; then
          echo "PERF_P3_WRITE_THRESHOLD S5 vu${vus} rep${repetition}"
        else
          echo "PERF_RUN_FAILED S5 vu${vus} rep${repetition} exit_code=${run_exit_code}"
          RUN_FAILED=1
