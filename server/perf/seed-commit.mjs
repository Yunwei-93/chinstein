...
async function verifyCommittedSeedWithFreshConnection(
  seedDate,
  expectedDataset,
  expectedSnapshot
) {
  if (
    expectedDataset?.classification !== "approved-perf-source" ||
    !expectedSnapshot
  ) {
    throw new PerfSafetyError(
      "Post-commit verification requires the expected PERF target state"
    );
  }
  ...
  try {
    ...
    if (clock.seedDate !== seedDate) {
      throw new PerfSafetyError(
        "Post-commit verification date differs from the seed date"
      );
    }
    ...
    const distribution = await verifySeededUserDistribution(
      client,
      seedDate
    );

    stage = "verify-post-commit-session-quality";
    const sessionQuality = await verifySeededSessionQuality(
      client,
      seedDate
    );

    stage = "verify-post-commit-approved-source";
    const approvedDataset = await assertApprovedSeedSourceDataset(
      client,
    ...
    if (!approvedSeedSourcesMatch(expectedDataset, approvedDataset)) {
      throw new PerfSafetyError(
        "Committed PERF identity differs from the pre-commit target"
      );
    }
    ...
    if (!coreSnapshotsMatch(expectedSnapshot, observedSnapshot)) {
      throw new PerfSafetyError(
        "Committed core data differs from the pre-commit target"
      );
    }
...
async function buildAndVerifySeedInCurrentTransaction(
    client,
    seedDate,
    passwordHash
) {
    ...
    try {
        const cleanup =
            await clearStagingDataset(client);
        ...
        if (finalClock.seedDate !== seedDate) {
            throw new PerfSafetyError(
                "Database date changed during the committed seed transaction"
            );
        }
...
export async function runCommittedSeed({
  ...
  if (
    !APPROVED_COMMIT_SOURCE_CLASSIFICATIONS.has(
      expectedSourceClassification
    )
  ) {
    throw new PerfSafetyError(
      "Committed seed requires one exact approved source classification"
    );
  }
  ...
  try {
    ...
    const fixture = await buildAndVerifySeedInCurrentTransaction(
      client,
      seedDate,
      passwordHash
    );
    ...
    if (targetDataset.sourceSeedDate !== seedDate) {
      throw new PerfSafetyError(
        "Target fixture date differs from the transaction seed date"
      );
    }
    ...
    if (coreSnapshotsMatch(sourceSnapshot, targetSnapshot)) {
      throw new PerfSafetyError(
        "Replacement target is indistinguishable from its source"
      );
    }
    ...
    if (preCommitClock.seedDate !== seedDate) {
      throw new PerfSafetyError(
        "Database date changed before the seed transaction committed"
      );
    }
    ...
    if (!commitStarted) {
      ...
      if (connection && transactionOpen) {
        rollbackConfirmed = await rollbackQuietly(
          connection.client
        );
        transactionOpen = false;
      }
      ...
      if (sourceDataset) {
        try {
          const restored =
            await verifySourceRestoredWithFreshConnection({
              expectedDataset: sourceDataset,
    ...
    if (!commitConfirmed) {
      ...
      const observedCommitOutcome =
        ...
        "committed-target-visible"
          ? "committed"
          : observation.classification ===
              "not-committed-source-restored"
            ? "not-committed"
            : "unknown";
