...
export async function setSyntheticStories(client) {
  ...
  `, ["generating"]);
    ...
    if (Number(state.characters) !== CHARACTER_COUNT) {
        throw new PerfSafetyError(
            "Character count changed; refusing to replace stories"
        );
    }

    if (Number(state.generating_stories) !== 0) {
        throw new PerfSafetyError(
            "Story generation is in progress; refusing to replace stories"
        );
    }
  ...
  `, [
    ...
    if (updated.rowCount !== CHARACTER_COUNT) {
        throw new PerfSafetyError(
            "Unexpected story update count; the caller must roll back"
        );
    }
...
export async function seedTodayConflictSessions(client, seedDate) {
    ...
    if (
        ...
        throw new PerfSafetyError(
            "Seed date does not match the current database date"
        );
    }
    ...
    if (
        ...
        Number(state.pool_b_history) !==
        POOLS.B.users * HISTORY_DAYS
    ) {
        throw new PerfSafetyError(
            "Pool B requires complete history and no current-day sessions"
        );
    }
    ...
    if (
        ...
        throw new PerfSafetyError(
            "Today's character is not the canonical ready synthetic fixture"
        );
    }
    ...
    if (inserted.rowCount !== TODAY_SESSIONS) {
        throw new PerfSafetyError(
            "Unexpected Pool B insert count; the caller must roll back"
        );
    }
    ...
    if (end.rows[0].wall_date !== seedDate) {
        throw new PerfSafetyError(
            "Database date changed during Pool B insertion; roll back"
        );
    }
...
export async function seedHistoricalSessions(client, seedDate) {
    ...
    if (
        ...
        throw new PerfSafetyError(
            "History requires complete mappings and an empty sessions table"
        );
    }
    ...
    if (inserted.rowCount !== HISTORICAL_SESSIONS) {
        throw new PerfSafetyError(
            "Unexpected history insert count; the caller must roll back"
        );
    }
    ...
    if (end.rows[0].wall_date !== seedDate) {
        throw new PerfSafetyError(
            "Database date changed during history insertion; roll back"
        );
    }
...
export async function createPerfUsers(client, passwordHash) {
    ...
    if (typeof passwordHash !== "string" || passwordHash.length === 0) {
        throw new PerfSafetyError(
            "A generated password hash is required to create perf users"
        );
    }
    ...
    if (
        Number(state.users) !== 0 ||
        Number(state.user_mappings) !== 0
    ) {
        throw new PerfSafetyError(
            "Users and user mappings must be empty before seeding"
        );
    }
    ...
    if (inserted.rowCount !== TOTAL_USERS) {
        throw new PerfSafetyError(
            "Unexpected user insert count; the caller must roll back"
        );
    }
    ...
    if (mapped.rowCount !== TOTAL_USERS) {
        throw new PerfSafetyError(
            "Unexpected user mapping count; the caller must roll back"
        );
    }
...
export async function createPerfMappings(client) {
  ...
  `);
    ...
    if (mapped.rowCount !== CHARACTER_COUNT) {
        throw new PerfSafetyError(
            "Unexpected character mapping count; the caller must roll back"
        );
    }
...
export async function clearStagingDataset(client) {
    ...
    if (Number(state.characters) !== CHARACTER_COUNT) {
        throw new PerfSafetyError(
            "Character count changed; refusing to clear staging data"
        );
    }

    if (Number(state.generating_stories) !== 0) {
        throw new PerfSafetyError(
            "Story generation is in progress; refusing to clear staging data"
        );
    }
