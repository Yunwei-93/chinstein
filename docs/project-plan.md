...
| PERF | Reproducible performance baseline and measured optimization | PERF-P0 frozen; PERF-P1 complete. The approved 8,100-user, 1,458,200-session, 365-character fixture passed the initial rollback/commit and the real repeat-seed rollback/confirmed commit, including fresh read-only post-commit verification. The first three local-only PERF-P2 checkpoints now freeze scenario, user-allocation, latency-threshold, response-classification, secret-free token-fixture, injectable builder, and 32-72-byte login-password contracts with 41 passing offline tests. No PERF-P2 HTTP measurement has run, and ECS remains intentionally drained at 0/0/0. |
